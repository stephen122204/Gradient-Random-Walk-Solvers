...
REPORTED = {
    ...
    'heat coupled total': (-0.493, -0.423),
    'heat coupled spread': (-0.507, -0.430),
}
RESULTS = []
...
def matrix(byK):
    Ks = sorted(byK)
    seeds = sorted(byK[Ks[0]])
    return np.array(Ks, float), np.array([[byK[K][s] for s in seeds] for K in Ks])


def fit(Ks, stat, loglog=True):
    return (
        float(np.polyfit(np.log(Ks), np.log(stat), 1)[0])
        if loglog
        else float(np.polyfit(Ks, stat, 1)[0])
    )
...
def interval(Ks, A, agg, joint, loglog=True):
    ...
    for _ in range(B):
        if joint:
            ...
            stat = np.array([agg(A[k, idx]) for k in range(len(Ks))])
        else:
...
def report(name, Ks, A, agg, loglog=True, tol=0.005):
    ...
    ind = interval(Ks, A, agg, False, loglog)
    jnt = interval(Ks, A, agg, True, loglog)
    rep = REPORTED.get(name)
    ok = rep is None or (
        abs(ind[0] - rep[0]) <= tol * max(1, abs(rep[0]))
        and abs(ind[1] - rep[1]) <= tol * max(1, abs(rep[1]))
    )
    RESULTS.append(
        dict(statistic=name, fit=est, independent=ind, joint=jnt, reported=rep, reproduced=ok)
    )
    print(
        f"{name:34s} fit {est:+.4g}  independent [{ind[0]:+.4g},{ind[1]:+.4g}]  joint [{jnt[0]:+.4g},{jnt[1]:+.4g}]"
        + (
            f"  reported [{rep[0]:+.4g},{rep[1]:+.4g}] {'reproduced' if ok else 'DIFFERS'}"
            if rep
            else ""
        )
    )
...
def heat_profiles():
    ...
    for r in csv.DictReader(
        open(os.path.join(ROOT, 'pinned_ensembles/heat_grid_paired/summary_by_N_paired.csv'))
    ):
        pinned[(r['treatment'], int(r['N']))] = r
    M = 300
    edges = np.linspace(0, L, M + 1)
    ...
    for N in Ns:
        ...
        ref_cpl = exact_heat_step(xg, T, x0, uL, uR, alpha)
        h_cpl = float(xg[1] - xg[0])
        for s in range(S):
            x_pos, w_arr, _, _, _ = _run_heat_one(N, alpha, T, dt, L, x0, uL, uR, base + s)
            _, u = reconstruct_cumulative(x_pos, w_arr, edges, uL)
            fixed_runs.append(u)
            order = np.argsort(x_pos)
            xs = x_pos[order]
            us = uL + np.cumsum(w_arr[order])
            coupled_runs.append(np.interp(xg, xs, us, left=uL, right=uR))
        u_f = np.array(fixed_runs)
        u_c = np.array(coupled_runs)
        for tr, ref, arr, hh in (
            ('fixed300', ref_c, u_f, h),
            ('fixed300e', ref_e, u_f, h),
            ('coupled', ref_cpl, u_c, h_cpl),
        ):
            got = _decompose(arr, ref, hh)[:3]
            row = pinned[(tr, N)]
            exp = [float(row['E_bias']), float(row['E_spread']), float(row['E_total'])]
            assert all(
                abs(g - e) <= 5.01e-9 for g, e in zip(got, exp)
            ), f'{tr} N={N} differs from pinned'
            d = arr - ref[None, :]
            grams[tr].append((d @ d.T) * hh)
        print(f"  heat profiles N={N} reproduce pinned statistics")
    lN = np.log(np.array(Ns, float))

    def stats_from(G, c):
        Sn = len(c)
        Et2 = float(c @ np.diag(G)) / Sn
        Eb2 = float(c @ G @ c) / Sn**2
        return Et2, max(Et2 - Eb2, 0.0), Eb2
            ...
            def run(joint):
                ...
                for _ in range(B):
                    ...
                    for G in grams[tr]:
                        ...
                        vals.append(stats_from(G, c)[k])
                    v = np.sqrt(np.maximum(np.array(vals), 1e-30))
                    out.append(float(np.polyfit(lN, np.log(v), 1)[0]))
                lo, hi = np.percentile(out, [2.5, 97.5])
                return float(lo), float(hi)

            est = float(
                np.polyfit(
                    lN, np.log(np.sqrt([stats_from(G, np.ones(S))[k] for G in grams[tr]])), 1
                )[0]
            )
            ind, jnt = run(False), run(True)
            name = f'heat {tr} {key}'
            rep = REPORTED.get(name)
            ok = rep is None or (abs(ind[0] - rep[0]) <= 0.005 and abs(ind[1] - rep[1]) <= 0.005)
            RESULTS.append(
                dict(
                    statistic=name, fit=est, independent=ind, joint=jnt, reported=rep, reproduced=ok
                )
            )
            print(
                f"{name:34s} fit {est:+.4g}  independent [{ind[0]:+.4g},{ind[1]:+.4g}]  joint [{jnt[0]:+.4g},{jnt[1]:+.4g}]"
...
def main():
    """Report both resampling schemes and optionally save their intervals."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--heat-profiles', action='store_true')
    ...
    Ks, A = matrix(h['l2'])
    report('heat E_total', Ks, A, rms)
    f = load(
        os.path.join(ROOT, 'pinned_ensembles/fhn_extended/per_run.csv'),
        'N',
        ['l2', 'center_err', 'speed_err', 'aligned_err'],
    )
    for col, name in [
        ('l2', 'reaction profile'),
        ('center_err', 'reaction front location'),
        ('speed_err', 'reaction speed'),
        ('aligned_err', 'reaction aligned profile'),
    ]:
        Ks, A = matrix(f[col])
        report(name, Ks, A, mean)
    bp = defaultdict(dict)
    path = Path(ROOT, 'pinned_ensembles/burgers_controls/decoupled_per_run.csv')
    with path.open(newline='') as stream:
        for r in csv.DictReader(stream):
            if r['part'] == 'A':
                bp[float(r['P'])][int(r['seed'])] = float(r['rmse_particle'])
    Ks, A = matrix(bp)
    report('Burgers particle RMSE vs P', Ks, A, mean)
    bd = load(
        os.path.join(ROOT, 'pinned_ensembles/burgers_controls/domain_per_run.csv'),
        'L',
        ['E_total_rmse'],
    )
    Ks, A = matrix(bd['E_total_rmse'])
    report('Burgers total RMSE vs L (linear)', Ks, A, mean, loglog=False, tol=0.02)
    if args.heat_profiles:
        heat_profiles()
    if args.json_path is not None:
        args.json_path.parent.mkdir(parents=True, exist_ok=True)
        args.json_path.write_text(json.dumps(RESULTS, indent=1))
