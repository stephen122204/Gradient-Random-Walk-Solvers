...
from study_t9_heat_two_step import (
    ALPHA,
    L,
    T,
    UL,
...
def reference_control(M, n=20_000_000, seed=11):
    ...
    print(
        f"    max |mean reconstruction - m| = {err.max():.3e}, pointwise sampling sd upper bound = {sd_point:.3e}, "
...
def identity_control(M, N=400, S=12, reps=4000, seed=7):
    ...
    Na = Nb = N // 2
    m_e = m_finite(be)
    g = m_finite(ce)  # bin-center comparison, so B_h > 0
    ...
    for k in pred:
        ...
        ok &= abs(z) < 4
        print(
            f"    {k:11s} measured {v.mean():.6e}  predicted {pred[k]:.6e}  "
            f"ratio {v.mean()/pred[k]:.5f}  ({z:+.2f} standard errors)"
        )
    assert (
        ok
    ), "identity control: a measured moment differs from its prediction by more than 4 standard errors"
    return ok
...
def seed_blocks(N, M_list, blocks, S=30, start=90000):
    ...
    print()
    for M in M_list:
        r = np.array([v[0] for v in out[M]])
        a = np.array([v[1] for v in out[M]])
        print(
            f"    M={M}: ratio mean {r.mean():.4f} sd {r.std(ddof=1):.4f} | "
...
if __name__ == '__main__':
    ...
    a = ap.parse_args()
    validate_particle_count(a.N)
    validate_particle_count(a.n_direct)
    if a.blocks < 2:
        ap.error("--blocks must be at least 2 to report a sample standard deviation")
    for M in (50, 200):
