#!/usr/bin/env python3
"""Regenerate all eight GRW manuscript figures from data or fixed-seed runs."""
...
def plot_heat_refinement(generated: list[str]) -> None:
    ...
    axis.loglog(counts, errors, "o-", color=GRW, label=r"$L_h^2$ error")
    axis.set_title("Heat GRW fixed-grid diagnostic")
...
def plot_fhn_diagnostics(arrays: dict[str, np.ndarray], generated: list[str]) -> None:
    ...
    figure.suptitle("Scalar FHN reaction-statistic diagnostics")
    figure.tight_layout(rect=(0, 0, 1, 0.96))
...
def plot_fhn_refinement(generated: list[str]) -> None:
    ...
    axis.loglog(counts, guide, ":", color=GUIDE, linewidth=1.4, label=r"$N^{-1/2}$ guide")
    axis.set_title("Scalar FHN GRW refinement")
    axis.set_xlabel(r"Particle count $N$")
...
def plot_burgers_diagnostics(arrays: dict[str, np.ndarray], generated: list[str]) -> None:
    ...
    axes[0, 0].set_title(r"Transformed field $\phi$")
    ...
    axes[0, 1].set_title(r"Recovery ratio $\phi_x/\phi$")
    ...
    axes[1, 0].set_title(r"Recovered field $u$")
    axes[1, 0].set_xlabel(r"$x$")
    ...
    axes[1, 1].plot(x, deterministic_error, color=DETERMINISTIC, label="Deterministic pipeline mismatch")
    axes[1, 1].plot(x, grw_error, color=GRW, linestyle="--", label="Additional GRW error")
    axes[1, 1].axhline(0.0, color=EXACT, linestyle=":", linewidth=1.0)
    axes[1, 1].set_title("Recovered-field error components")
    axes[1, 1].set_xlabel(r"$x$")
    axes[1, 1].set_ylabel("Error")
    axes[1, 1].legend(loc="best", fontsize=8)

    figure.suptitle("Cole–Hopf Burgers pipeline diagnostics")
    figure.tight_layout(rect=(0, 0, 1, 0.96))
...
def plot_burgers_domain(generated: list[str]) -> None:
    ...
    figure, axis = plt.subplots(figsize=(4.8, 3.6))
    axis.plot(lengths, deterministic, "o-", color=DETERMINISTIC, label="Deterministic pipeline mismatch")
    axis.plot(lengths, grw, "s-", color=GRW, label="Additional GRW reconstruction error")
    axis.set_title("Cole–Hopf Burgers domain sensitivity")
...
def compute_metrics(arrays: dict[str, np.ndarray]) -> dict[str, object]:
    """Compute the representative error norms reported in the manuscript table."""
    heat = {
        "N": 50000,
