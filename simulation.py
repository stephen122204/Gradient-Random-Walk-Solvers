...
def random_walk(globs, diff_constant, time_step):
    ...
    2 * alpha * dt, i.e. sigma = sqrt(2 * alpha * dt), matching the standard random-walk step.
...
def simulate_fitzhugh_nagumo(globs, config):
    ...
    Routes to the scalar GRW (simulate_fitzhugh_nagumo_grw)
    when config.fhn_ic_type indicates a supported IC (steady_solution,
    ...
    On the GRW-feasibility (main) branch, the scalar GRW is the
...
def simulate_burgers_lagrangian(globs, config):
    ...
    It is kept for comparison.  The preferred method is Cole-Hopf GRW.
    """
    dt  = config.time_step
...
def simulate_burgers_cole_hopf_grw(globs, config):
    ...
    # Dirichlet reflection is retained as the simplest consistent choice.
    n_steps = int(config.total_time / dt)
...
def simulate_burgers_direct_grw(globs, config):
    ...
    THIS IS NOT THE PRIMARY SOLVER.  It is included to show
    that the direct GRW method for Burgers is severely noisy and
