...
writes Figure_11.pdf. The Gaussian and delta approximations give a finite-ensemble fluctuation scale
from the reconstruction covariance, so that agreement is
...
from study_t9_heat_two_step import grid, covariance, m_finite, u_inf, M_LIST, N_LIST  # noqa: E402


def main():
    """Read the selected study outputs, report comparisons, and draw Figure 11."""
    apply_paper_style()
    ...
    pred = json.loads(Path(DATA, 'predictions.json').read_text())
    ens = json.loads(Path(DATA, 'ensembles.json').read_text())
    ...
    def predicted(M, ref, conv, N, stat):
        p = pM[M]
        B2 = p[f'{ref}_{conv}']['B2']
        V = p['V_h']
        return {
            'E_total': sqrt(B2 + V / N),
            'E_bias': sqrt(B2 + V / (N * S)),
            'E_spread': sqrt((S - 1) / S * V / N),
        }[stat]

    def sd_total(M, ref, conv, N):
        """Approximate SD of measured E_total from the reconstruction covariance.
            ...
            for conv in ('edge', 'center'):
                sel = sorted(
                    [
                        r
                        for r in rows
                        if r['M'] == M and r['reference'] == ref and r['convention'] == conv
                    ],
                    key=lambda r: r['N'],
                )
                z = [
                    (r['E_total'] - predicted(M, ref, conv, r['N'], 'E_total'))
                    / sd_total(M, ref, conv, r['N'])
                    for r in sel
                ]
                devs += z
                worst = max(worst, max(abs(np.array(z))))
    ...
    for M in M_LIST:
        ...
        for conv in ('center', 'edge'):
            B = p[f'finite_{conv}']['B']
            Ns = p[f'finite_{conv}']['N_star']
            sel = sorted(
                [
                    r
                    for r in rows
                    if r['M'] == M and r['reference'] == 'finite' and r['convention'] == conv
                ],
                key=lambda r: r['N'],
            )
            ...
            s += (
                f" N*={Ns:8.0f} nearest N={near['N']:6d}: measured {near['E_total']:.5f} predicted {predicted(M,'finite',conv,near['N'],'E_total'):.5f}"
                if Ns
                else "  no floor (B=0), sampling only"
            )
            print(s)
    ...
    for stat in ('E_total', 'E_spread', 'E_bias'):
        r_ = [
            r[stat] / predicted(r['M'], r['reference'], r['convention'], r['N'], stat) for r in rows
        ]
    ...
    for ax, ref, title in (
        (axes[0], 'finite', '(a) finite-interval reference'),
        ...
        for ci, M in enumerate(M_LIST):
            for conv, mk, ls in (('center', 'o', '-'), ('edge', 's', '--')):
                ...
                ax.plot(
                    Nline,
                    [predicted(M, ref, conv, n, 'E_total') for n in Nline],
                    ls,
                    color=colors[ci],
                    lw=1.0,
                )
    ...
    if os.path.exists(f'{DATA}/validation.json'):
        ...
        print(
            f"model rules out the bin-center convention before running: floor {spec['center_convention']['B']:.5f} exceeds the target"
        )
        for r in val['rows']:
            if r['M'] == M and r['reference'] == 'infinite':
                pr = predicted(M, 'infinite', r['convention'], N, 'E_total')
                print(
                    f"  {r['convention']:6s} N={N}: measured {r['E_total']:.5f}  predicted {pr:.5f}  "
