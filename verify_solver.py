...
a trusted benchmark:
  ...
  burgers -- three modes depending on config.burgers_mode:
    cole_hopf_grw (default): Cole-Hopf transform reduces Burgers to a heat equation
        ...
        reference is used.
    direct_grw: diagnostic path reproducing noise discussion.
...
def run_burgers(cfg, output_dir, do_save_data, ref_factor):
    ...
    print("\n" + "=" * 68)
    mode_labels = {
        'cole_hopf_grw': 'Cole-Hopf GRW  (Burgers -> heat via Cole-Hopf)',
        'direct_grw':    'Direct GRW  (diagnostic: expected to be noisy)',
    ...
    if mode == 'direct_grw':
        ...
        print(f"    relative to the FD reference are expected and confirm the")
...
def run_fhn(cfg, output_dir, do_save_data, ref_factor):
    ...
    Scalar GRW ICs (steady_solution, nonsmooth, discontinuous):
      Compare the scalar GRW result to the exact traveling-wave solution
